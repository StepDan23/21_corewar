...
/*   Updated: 2019/04/04 00:04:49 by artemiy          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	op_tab[1] = (t_op){"live", 1, {T_DIR}, 1, 10, "alive", 0, 0};
	op_tab[2] = (t_op){"ld", 2, {T_DIR | T_IND, T_REG}, 2, 5, "load", 1, 0};
	op_tab[3] = (t_op){"st", 2, {T_REG, T_IND | T_REG}, 3, 5, "store", 1, 0};
	...
	op_tab[9] = (t_op){"zjmp", 1, {T_DIR}, 9, 20, "jump if zero", 0, 1};
	...
	"store index", 1, 1};
	op_tab[12] = (t_op){"fork", 1, {T_DIR}, 12, 800, "fork", 0, 1};
	op_tab[13] = (t_op){"lld", 2, {T_DIR | T_IND, T_REG}, 13, 10, "long load", 1, 0};
	op_tab[14] = (t_op){"lldi", 3, {T_REG | T_DIR | T_IND, T_DIR | T_REG, T_REG}, 14, 50,
		"long load index", 1, 1};
	op_tab[15] = (t_op){"lfork", 1, {T_DIR}, 15, 1000, "long fork", 0, 1};
	op_tab[16] = (t_op){"aff", 1, {T_REG}, 16, 2, "aff", 1, 0};
